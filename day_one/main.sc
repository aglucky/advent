...
def getNums(line: String): List[Int] = {
  ...
  val line_numbers: List[Int] = lowercase_line.zipWithIndex.flatMap {
    case (letter, index) =>
      if (letter.isLetter) {
        // Check if letter is start of num_word, will return None if no matches
        word_nums.collectFirst {
          case (word, num) if lowercase_line.startsWith(word, index) =>
            num // Add num if letter is start of num's word form
        }
...
def dayOne(): Int = {
  ...
  val sum: Int = content
    ...
    .map(x =>
      if (x.length == 1) s"${x.head}${x.head}" else s"${x.head}${x.last}"
